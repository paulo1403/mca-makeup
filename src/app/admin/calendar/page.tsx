      ...
      <div className="bg-surface rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-hidden border border-[var(--color-border)]">
        ...
        <div className="p-6 border-b border-[var(--color-border)]">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xl font-semibold text-heading">{appointment.clientName}</h3>
              <span
                ...
                <div className="space-y-2">
                  {appointment.services.map((service, index) => (
                    <div key={index} className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-medium text-main">S/ {service.price}</span>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="font-medium text-heading">{appointment.service}</p>
              )}
            </div>
          </div>
...
export default function AdminCalendar() {
  ...
  const appointmentsByDay = useMemo(() => {
    ...
    map.forEach((appointments) => {
      appointments.sort((a, b) => a.startDate.getTime() - b.startDate.getTime());
    });
        ...
        <div className="bg-surface rounded-lg shadow-sm p-4 md:p-6 mb-6 border border-[var(--color-border)]">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl md:text-2xl font-bold text-heading">Calendario de Citas</h1>
                <p className="text-sm md:text-base text-muted">Vista mensual personalizada</p>
              </div>
            </div>
          ...
          <div className="space-y-4">
            {monthDays
              .filter((day) => {
                ...
                return dayAppointments.length > 0 && isSameMonth(day, currentDate);
              })
              .map((day) => {
                const dayKey = format(day, "yyyy-MM-dd");
                const dayAppointments = appointmentsByDay.get(dayKey) || [];
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <h3 className="font-semibold">{format(day, "EEEE d", { locale: es })}</h3>
                          <p className={`text-sm ${isTodayDate ? "text-white/80" : "text-muted"}`}>
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-main mb-2">{appointment.time}</p>
            ...
            {monthDays.filter((day) => {
              ...
              <div className="bg-surface rounded-lg shadow-sm p-8 text-center border border-[var(--color-border)]">
                ...
                <h3 className="text-lg font-medium text-heading mb-2">No hay citas este mes</h3>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      {dayAppointments.slice(0, isMobile ? 1 : 3).map((appointment) => (
                        <div
                          key={appointment.id}
                          onClick={() => handleAppointmentClick(appointment)}
                        ...
                        >
                          ...
                          {!isMobile && (
                            <>
                              ...
                              <div className="text-xs opacity-90">S/ {appointment.totalPrice}</div>
                            </>
                          )}
                        </div>
                      ))}
