...
function AppointmentsContent() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["appointments", "unified", qp],
    queryFn: async () => {
  ...
  const onDelete = (id: string) => del.mutate(id, { onSuccess: () => { toast.success("Cita eliminada"); setSelId(null); }, onError: (e) => toast.error(e.message) });
  ...
  const completedRevenue = apps.filter((a) => a.status === "COMPLETED").reduce((s, a) => s + (a.totalPrice || 0), 0);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="shrink-0 space-y-2 pb-3">
        ...
        <div className="flex items-center gap-2 flex-wrap text-[11px]">
          ...
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[color:var(--color-primary)]/10 text-[color:var(--color-primary)]">
            S/ <strong>{new Intl.NumberFormat("es-PE", { style: "currency", currency: "PEN", maximumFractionDigits: 0 }).format(completedRevenue)}</strong> mes
          </span>
          {pag && <span className="text-[color:var(--color-muted)] ml-auto">{pag.total} total</span>}
        </div>
      ...
      <div className="flex-1 flex gap-0 min-h-0">
        {/* List */}
        <div className={`flex-1 min-w-0 overflow-auto ${selAppt ? "hidden lg:block lg:w-[56%] lg:flex-none lg:border-r lg:border-[color:var(--color-border)] lg:pr-3" : ""}`}>
          {isLoading ? (
                          ...
                          return (
                            <button key={a.id} onClick={() => setSelId(active ? null : a.id)}
                              ...
                              <div className="px-3 py-2">
                                ...
                                <div className="flex items-center gap-1.5">
                                  <span className="text-xs font-medium text-[color:var(--color-heading)] truncate max-w-[180px]">{a.clientName}</span>
        ...
        {selAppt && (
          <div className="hidden lg:flex lg:w-[44%] lg:flex-none lg:flex-col lg:pl-3 lg:overflow-y-auto">
            <DetailPanel
              a={selAppt}
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3 text-xs">
        ...
        <div>
          ...
          <div className="flex items-start gap-1.5 text-[color:var(--color-body)]">
            ...
            <span>{isStudio ? "Studio — Av. Bolívar 1073, Pueblo Libre" : [a.address, a.district, a.addressReference ? `Ref: ${a.addressReference}` : ""].filter(Boolean).join(", ")}</span>
          </div>
          {!isStudio && a.address && (
