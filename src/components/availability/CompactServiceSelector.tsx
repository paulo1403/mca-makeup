...
export default function CompactServiceSelector({
  value,
  onChangeAction,
  className = "",
  showPrice = false,
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div
          className="flex items-center gap-2 overflow-x-auto -mx-1 px-1 lg:overflow-visible lg:flex-wrap lg:mx-0"
      ...
      <div className="grid grid-cols-1 gap-2 sm:gap-3 lg:gap-5 items-stretch">
        {loading ? (
          Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-24 sm:h-28 rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] animate-pulse"
            />
          ))
        ) : filtered.length === 0 ? (
          ...
          (showAll ? filtered : filtered.slice(0, INITIAL_LIMIT)).map((svc) => {
            ...
            return (
              <div
                ...
                className={`text-left p-3 sm:p-4 lg:p-5 rounded-xl transition-all duration-200 border w-full flex flex-col lg:min-h-[200px] ${
                  selected
