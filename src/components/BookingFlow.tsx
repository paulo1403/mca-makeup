...
export default function BookingFlow() {
  const methods = useForm<BookingData>({ resolver: zodResolver(BookingSchema), defaultValues: { name: '', phone: '', email: '', selectedServices: [], locationType: 'HOME', date: undefined as unknown as Date, timeSlot: '', message: '', additionalNotes: '', agreedToTerms: false } })
  ...
  const sendBooking = useMutation({
    ...
    onSuccess: (data: { pricing?: Pricing }) => {
      const defaults = { name: '', phone: '', email: '', selectedServices: [], locationType: 'HOME' as const, date: undefined as unknown as Date, timeSlot: '', message: '', additionalNotes: '', agreedToTerms: false }
        ...
        <div className="container mx-auto px-3 md:px-6 max-w-6xl">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="mt-6 bg-card p-4 sm:p-6 rounded-lg shadow-md">
                ...
                {currentStep === 5 && (
                  <div className="mt-8 pt-6 border-t border-border">
                    <h4 className="font-playfair text-lg mb-4">Resumen de tu reserva</h4>
                    <BookingSummary />
                  </div>
                )}

                <div className={`mt-6 flex flex-col gap-3 sm:flex-row sm:items-center ${currentStep === total ? 'sm:justify-end' : 'sm:justify-between'}`}>
                    ...
                    {currentStep < total ? (
                      ...
                      >
                        <span className="inline-flex items-center gap-2">
                          ...
                          {sendBooking.isPending ? 'Enviando…' : 'Confirmar y Enviar Solicitud de Cita'}
                        </span>
                      </Button>
                    )}
                  </div>
            ...
            <div className="hidden lg:block">
              <ContactQuickPanel variant="sidebar" />
            </div>
          </div>
        </div>
      </section>
