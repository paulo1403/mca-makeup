...
async function testAppointmentsPage(page, viewType) {
  ...
  const headerH1 = await page.locator('h1').first().isVisible({ timeout: 3000 }).catch(() => false);
  const h1Text = await page.locator('h1').first().textContent().catch(() => '');
  const kpisVisible = await page.locator('text=Confirmadas').isVisible({ timeout: 3000 }).catch(() => false);
  const statsToggleBtn = await page.locator('button:has-text("Ver estadísticas")').isVisible({ timeout: 3000 }).catch(() => false);
  const searchbox = await page.locator('input[placeholder*="Buscar"]').isVisible({ timeout: 3000 }).catch(() => false);
  
  console.log(`  H1 text: ${h1Text}`);
  console.log(`  Header visible: ${headerH1}, KPIs: ${kpisVisible}, Stats toggle: ${statsToggleBtn}, Search: ${searchbox}`);
  console.log(`  Page has Citas: ${hasGestionCitas}, Confirmadas: ${hasConfirmadas}, Buscar: ${hasSearchInput}`);
  ...
  if (statsToggleBtn) {
    ...
    const detailsInitial = await page.locator('text=Total citas:').isVisible().catch(() => false);
    
    await statsButton.click();
    await page.waitForTimeout(300);
    
    const detailsAfterClick = await page.locator('text=Total citas:').isVisible().catch(() => false);
